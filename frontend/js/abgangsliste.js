...
// ---------- Abgangsliste: Etiketten (Avery Zweckform L6037) ----------
const LABEL_SHEET_SIZE = 189;
let LABEL_START_POS = 1;

function renderLabelGrid() {
  const grid = document.getElementById('label-position-grid');
  const cells = [];
  for (let i = 1; i <= LABEL_SHEET_SIZE; i++) {
...
function updateLabelGridSelection() {
  ...
  document.getElementById('label-start-text').textContent =
    `Start bei Etikett Nr. ${LABEL_START_POS} von ${LABEL_SHEET_SIZE} (auf einen freien Etikett klicken, um die Startposition zu ändern).`;
}
...
function downloadLabelsPdf() {
  ...
  window.location.href = `/api/projects/${CURRENT_PROJECT}/export-labels.pdf?source=${source}&start=${LABEL_START_POS}&debug=${debug}`;
}
